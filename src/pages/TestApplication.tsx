...
import { CheckIcon, ClockIcon, ChevronRightIcon, ChevronLeftIcon, AlertCircleIcon, BookOpenIcon, PlayIcon, ListChecksIcon, RotateCcwIcon } from 'lucide-react';
...
function InstructionsScreen({
  ...
  const instructions = test?.description ? test.description.split(/\.\s+/).filter((s: string) => s.trim().length > 10) : [];
  ...
  return <div className="min-h-screen flex items-center justify-center p-6" style={{
    background: `linear-gradient(160deg, ${TR.navy} 0%, ${TR.blue} 60%, #1e3a8a 100%)`
  }}>
      <div className="w-full max-w-2xl">
        ...
        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden">
          ...
          <div className="grid grid-cols-3 divide-x divide-gray-100 border-b border-gray-100">
            ...
            label: 'Duración',
            value: `${test?.duration || 45} min`,
            icon: ClockIcon
          }, {
            label: 'Formato',
            value: test?.format || 'Selección múltiple',
            icon: CheckIcon
          }].map((s) => <div key={s.label} className="px-6 py-5 text-center">
                <s.icon className="w-5 h-5 mx-auto mb-1.5" style={{
              color: TR.blue
            }} />
          ...
          {savedProgress && <div className="mx-6 mt-5 p-4 rounded-2xl border-2" style={{
          borderColor: `${TR.green}40`,
          background: `${TR.green}08`
        }}>
              <div className="flex items-start gap-3">
                <RotateCcwIcon className="w-5 h-5 flex-shrink-0 mt-0.5" style={{
              color: TR.green
            }} />
                <div className="flex-1">
                  <p className="font-black text-sm" style={{
                color: TR.greenDark
              }}>Tienes un progreso guardado</p>
          ...
          <div className="px-8 py-6">
            <h2 className="text-sm font-black uppercase tracking-widest mb-4" style={{
            color: TR.blue
          }}>
              Instrucciones
            </h2>
            <div className="space-y-3">
              {(instructions.length > 0 ? instructions : ['Lea cada pregunta cuidadosamente antes de seleccionar una respuesta.', 'Marque una sola opción por pregunta: A, B, C o D.', 'Su progreso se guarda automáticamente — puede cerrar la sesión y continuar después.', 'Este examen es para detección de necesidades de formación.']).map((inst: string, i: number) => <div key={i} className="flex items-start gap-3">
                  <div className="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 text-xs font-black text-white" style={{
            ...
            <div className="mt-5 p-4 rounded-2xl flex items-start gap-3" style={{
            background: `${TR.blue}08`,
            border: `1px solid ${TR.blue}15`
          }}>
              <AlertCircleIcon className="w-4 h-4 flex-shrink-0 mt-0.5" style={{
              color: TR.blue
            }} />
              <p className="text-xs text-gray-600 leading-relaxed">
                {savedProgress ? 'Puede continuar donde lo dejó o iniciar de nuevo. Si inicia de nuevo, el progreso anterior se perderá.' : 'Una vez que presione Iniciar Examen, el tiempo comenzará a correr. Su progreso se guardará automáticamente.'}
              </p>
            </div>
          </div>
          ...
          <div className="px-8 pb-8 space-y-3">
            {savedProgress && <button onClick={onResume} className="w-full py-4 rounded-2xl font-black text-white text-base shadow-lg hover:opacity-90 flex items-center justify-center gap-3" style={{
            background: `linear-gradient(135deg, ${TR.green}, ${TR.greenDark})`,
            boxShadow: `0 6px 20px ${TR.green}40`
          }}>
                <RotateCcwIcon className="w-5 h-5" />
                Continuar desde donde lo dejé ({answeredCount}/{questions.length})
              </button>}
            <button onClick={onStart} className="w-full py-3.5 rounded-2xl font-bold text-sm hover:opacity-90 flex items-center justify-center gap-2" style={{
...
export function TestApplication({
  ...
  const handleFinish = async (fromTimer = false) => {
    ...
    try {
      ...
      if (candidateId) {
        const {
          data: allScores
        } = await supabase.from('results').select('score').eq('user_name', String(candidateId));
        const avg = allScores?.length ? Math.round(allScores.reduce((s: number, r: any) => s + (r.score || 0), 0) / allScores.length) : finalScore;
  ...
  if (loading) return <div className="min-h-screen flex flex-col items-center justify-center" style={{
  ...
  if (!test) return <div className="min-h-screen flex flex-col items-center justify-center" style={{
    background: `linear-gradient(135deg, ${TR.navy}, ${TR.blue})`
  }}>
  ...
  // ── Instrucciones ──────────────────────────────────────────────
  if (phase === 'instructions') return <InstructionsScreen test={test} questions={questions} savedProgress={savedProgress} onStart={handleStart} onResume={handleResume} />;
    ...
    return <div className="min-h-screen flex items-center justify-center p-6" style={{
      ...
      <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden">
        <div className="px-8 py-8 text-center" style={{
          background: timedOut ? '#fef2f215' : `${TR.green}12`,
          borderBottom: `1px solid ${timedOut ? '#fecaca' : TR.green + '20'}`
        }}>
          <div className="w-20 h-20 mx-auto mb-4">
            <div className="w-20 h-20 rounded-full flex items-center justify-center shadow-lg mx-auto" style={{
              background: timedOut ? 'linear-gradient(135deg, #dc2626, #991b1b)' : `linear-gradient(135deg, ${TR.green}, ${TR.greenDark})`
            }}>
              {timedOut ? <svg className="w-9 h-9 text-white" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path d="M12 6v6l4 2m6-2a10 10 0 11-20 0 10 10 0 0120 0z" strokeLinecap="round" strokeLinejoin="round" /></svg> : <svg className="w-9 h-9 text-white" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" strokeLinecap="round" strokeLinejoin="round" /></svg>}
            </div>
          </div>
          ...
          <p className="text-sm font-semibold" style={{
            color: timedOut ? '#dc2626' : TR.green
          }}>
            {String(test.name || '')}
          </p>
        </div>
        ...
        <div className="px-8 py-6 space-y-4">
          {timedOut && unansweredCount > 0 && <div className="p-3 rounded-xl bg-red-50 border border-red-100 flex items-start gap-2.5">
          ...
          <div className={`grid gap-3 ${timedOut && unansweredCount > 0 ? 'grid-cols-3' : 'grid-cols-2'}`}>
            ...
            {timedOut && unansweredCount > 0 && <div className="p-3 rounded-xl text-center bg-red-50 border border-red-100">
                <p className="text-xs text-gray-400 mb-0.5">Incorrectas</p>
                <p className="text-xl font-black text-red-500">{unansweredCount}</p>
              </div>}
            <div className="p-3 rounded-xl text-center" style={{
              background: timedOut ? '#fef2f2' : `${TR.green}08`,
              border: `1px solid ${timedOut ? '#fecaca' : TR.green + '20'}`
            }}>
        ...
        <div className="px-8 pb-7">
          <button onClick={() => window.location.reload()} className="w-full py-3 rounded-xl text-white font-bold text-sm shadow-md hover:opacity-90" style={{
            background: `linear-gradient(135deg, ${TR.blue}, ${TR.navy})`
          }}>
            Volver al Portal
          </button>
  ...
  return <div className="min-h-screen flex flex-col" style={{
    background: `linear-gradient(160deg, ${TR.navy} 0%, ${TR.blue} 60%, #1e3a8a 100%)`
      ...
      background: 'rgba(26,45,107,0.88)',
      borderBottom: '1px solid rgba(255,255,255,0.08)'
        ...
        <div className="h-0.5 bg-white/10">
          ...
          width: `${progress}%`,
          background: `linear-gradient(90deg, ${TR.green}, ${TR.greenDark})`
        }} />
        </div>
      </header>
        ...
        <div className="w-full max-w-2xl space-y-4">
          ...
          <div className="bg-white rounded-3xl shadow-2xl overflow-hidden">
            ...
            <div className="px-8 pb-8 space-y-2.5">
              ...
              return <button key={i} onClick={() => handleAnswer(i)} className="w-full text-left p-4 rounded-2xl border-2 flex items-center gap-4 transition-all duration-150" style={{
                ...
                background: isSelected ? `${TR.blue}10` : '#fafafa'
                  ...
                  color: isSelected ? TR.navy : '#374151'
                }}>
                      {opt.label}
                    </span>
                  </button>;
          ...
          <div className="flex items-center justify-between">
            ...
            setCurrent((c) => c - 1);
          }} className="flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold text-sm" style={{
            ...
            {current < questions.length - 1 ? <button onClick={next} disabled={selectedIndex === undefined} className="flex items-center gap-2 px-6 py-2.5 rounded-xl font-bold text-sm text-white disabled:opacity-40" style={{
            background: selectedIndex !== undefined ? `linear-gradient(135deg, ${TR.blue}, ${TR.navy})` : 'rgba(255,255,255,0.15)'
          }}>
                  Siguiente <ChevronRightIcon className="w-4 h-4" />
                </button> : <button onClick={() => handleFinish()} disabled={saving || selectedIndex === undefined} className="flex items-center gap-2 px-6 py-2.5 rounded-xl font-bold text-sm text-white disabled:opacity-40" style={{
            background: `linear-gradient(135deg, ${TR.green}, ${TR.greenDark})`,
            boxShadow: `0 4px 14px ${TR.green}40`
          }}>
                  {saving ? <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Guardando...</> : <><CheckIcon className="w-4 h-4" /> Enviar Evaluación</>}
                </button>}
          </div>
          ...
          <div className="flex flex-wrap gap-1.5 justify-center pt-1">
            ...
            return <button key={q.id} onClick={() => setCurrent(i)} className="w-7 h-7 rounded-lg text-xs font-bold transition-all" style={{
