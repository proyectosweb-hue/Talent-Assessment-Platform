...
import { SearchIcon, PlusIcon, DownloadIcon, EyeIcon, FilterIcon, Loader2Icon, AlertCircleIcon, LockIcon, UsersIcon, CheckCircle2Icon, ClockIcon, BriefcaseIcon, XCircleIcon, EditIcon, ArchiveIcon, ArchiveRestoreIcon, SaveIcon, XIcon } from 'lucide-react';
...
function StatCard({
  ...
  return <div className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm flex items-center gap-3">
...
function EditCandidateModal({
  ...
  useEffect(() => {
    ...
    supabase.from('positions').select('id, name').or('archived.eq.false,archived.is.null').order('name').then(({
      data
    }) => setPositions(data || [])).finally(() => setLoadingPos(false));
  }, []);
      ...
      <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSave} className="px-7 py-5 space-y-4">
          ...
          <div>
            ...
            <select value={form.position} onChange={(e) => set('position', e.target.value)} className={inputClass} disabled={loadingPos}>
              <option value="">{loadingPos ? 'Cargando puestos...' : 'Sin puesto asignado'}</option>
              {positions.map((p) => <option key={p.id} value={p.name}>{p.name}</option>)}
              {!loadingPos && positions.length === 0 && <option disabled>No hay puestos activos</option>}
            </select>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {STATUS_OPTIONS.map((opt) => <button key={opt.value} type="button" onClick={() => set('status', opt.value)} className="py-2.5 rounded-xl text-xs font-bold border-2 transition-all" style={{
          ...
          <div className="flex gap-3 pt-2 border-t border-gray-100">
            <button type="button" onClick={onClose} className="flex-1 py-2.5 border border-gray-200 rounded-xl font-semibold text-sm text-gray-600 hover:bg-gray-50 transition-colors">
              Cancelar
            </button>
            <button type="submit" disabled={saving} className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl font-bold text-sm text-white shadow-md disabled:opacity-60 transition-all hover:opacity-90" style={{
            background: `linear-gradient(135deg, ${TR.green}, ${TR.greenDark})`,
            boxShadow: `0 4px 12px ${TR.green}40`
          }}>
              {saving ? <><Loader2Icon className="w-4 h-4 animate-spin" /> Guardando...</> : <><SaveIcon className="w-4 h-4" /> Guardar</>}
            </button>
          </div>
        </form>
      </div>
...
function ArchiveCandidateModal({
  ...
  return <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50" onClick={onCancel}>
      <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        background: isArchiving ? 'linear-gradient(135deg, #b45309, #92400e)' : `linear-gradient(135deg, ${TR.greenDark}, ${TR.green})`
      }}>
        ...
        <div className="p-6">
          <p className="text-gray-600 text-sm leading-relaxed">
            {isArchiving ? <>¿Estás seguro de archivar a <strong className="text-gray-900">"{String(candidate.name)}"</strong>? No aparecerá en la lista activa.</> : <>¿Deseas restaurar a <strong className="text-gray-900">"{String(candidate.name)}"</strong> a la lista activa?</>}
          </p>
        </div>
...
export function Candidates({
  ...
  const fetchCandidates = useCallback(async () => {
    try {
      ...
      const {
        data,
        error: err
      } = await supabase.from('candidates').select('*').order('id', {
        ascending: false
      });
      if (err) throw err;
      setCandidates(data || []);
  ...
  const filtered = pool.filter((c) => c.name.toLowerCase().includes(searchTerm.toLowerCase()) || c.email.toLowerCase().includes(searchTerm.toLowerCase()) || c.position && c.position.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  return <div className="p-6 space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5">
            ...
            {isReadonly && <span className="ml-2 inline-flex items-center gap-1 px-2 py-0.5 bg-yellow-100 text-yellow-700 rounded-full text-xs font-semibold">
                <LockIcon className="w-3 h-3" />Solo lectura
              </span>}
          </p>
        </div>
        <div className="flex gap-2">
          <button onClick={handleExport} className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 text-sm font-semibold text-gray-600 shadow-sm transition-colors">
            <DownloadIcon className="w-4 h-4" /> Exportar
          </button>
          {!isReadonly && <button onClick={onAddNewCandidate} className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold text-white shadow-md transition-all hover:opacity-90" style={{
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-3 flex gap-3">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Buscar por nombre, correo o puesto..." className="w-full pl-9 pr-9 py-2 border border-gray-200 rounded-lg outline-none text-sm bg-gray-50/50 focus:bg-white transition-colors" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        ...
        {/* Toggle archivados */}
        <button onClick={() => setShowArchived(!showArchived)} className={`flex items-center gap-2 px-4 py-2 border rounded-lg text-sm font-bold transition-colors ${showArchived ? 'bg-yellow-50 border-yellow-200 text-yellow-700' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}>
          <ArchiveIcon className="w-4 h-4" />
          {showArchived ? `Activos (${active.length})` : `Archivados (${archived.length})`}
        </button>

        <button onClick={() => setShowFilterModal(true)} className="flex items-center gap-2 px-4 py-2 border border-gray-200 rounded-lg text-sm font-semibold text-gray-600 hover:bg-gray-50 transition-colors">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
        {loading ? <div className="flex flex-col items-center justify-center h-64">
          ...
          </div> : error ? <div className="flex flex-col items-center justify-center h-64 text-red-400 p-6 text-center">
            <AlertCircleIcon className="w-12 h-12 mb-3" />
            <p className="font-bold text-red-600">Error de conexión</p>
            <p className="text-sm mt-1 opacity-80">{error}</p>
            <button onClick={fetchCandidates} className="mt-4 text-sm font-bold underline" style={{
          color: TR.blue
        }}>Reintentar</button>
          </div> : <>
            {/* Header tabla */}
            <div className="grid grid-cols-12 px-6 py-3 text-[11px] font-bold uppercase tracking-widest text-gray-400 border-b border-gray-100" style={{
          background: showArchived ? '#fffbeb' : '#fafafa'
        }}>
            ...
            <div className="divide-y divide-gray-50">
              {paginated.length === 0 ? <div className="flex flex-col items-center justify-center py-16 text-gray-300">
                ...
                </div> : paginated.map((candidate) => {
            const compat = Number(candidate.compatibility) || 0;
            const color = getCompatColor(compat);
            return <div key={candidate.id} className={`grid grid-cols-12 px-6 py-4 items-center transition-colors group cursor-default ${showArchived ? 'hover:bg-yellow-50/30 bg-yellow-50/10' : 'hover:bg-blue-50/30'}`}>

                    {/* Candidato */}
                    <div className="col-span-4 flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white text-xs font-bold flex-shrink-0" style={{
                  background: showArchived ? '#94a3b8' : `linear-gradient(135deg, ${TR.blue}, ${TR.navy})`
                }}>
                  ...
                  color
                    ...
                    <div className="col-span-1 flex justify-end items-center gap-1">
                      ...
                      {!showArchived && <button onClick={() => onViewCandidate(candidate)} title="Ver perfil" className="p-1.5 rounded-lg transition-all opacity-0 group-hover:opacity-100 hover:bg-blue-100" style={{
                  color: TR.blue
                }}>
                      ...
                      {!isReadonly && !showArchived && <button onClick={() => setEditingCandidate(candidate)} title="Editar candidato" className="p-1.5 rounded-lg transition-all opacity-0 group-hover:opacity-100 hover:bg-green-100" style={{
                  color: TR.green
                }}>
                      ...
                      {/* Archivar / Restaurar */}
                      {!isReadonly && <button onClick={() => setArchiveTarget({
                  candidate,
                  mode: showArchived ? 'unarchive' : 'archive'
                })} title={showArchived ? 'Restaurar candidato' : 'Archivar candidato'} className={`p-1.5 rounded-lg transition-all opacity-0 group-hover:opacity-100 ${showArchived ? 'hover:bg-green-100' : 'hover:bg-yellow-100'}`} style={{
                  color: showArchived ? TR.green : '#b45309'
                }}>
                          {showArchived ? <ArchiveRestoreIcon className="w-3.5 h-3.5" /> : <ArchiveIcon className="w-3.5 h-3.5" />}
                        </button>}
                    </div>
                  </div>;
          })}
      ...
      {/* Modales */}
      {editingCandidate && <EditCandidateModal candidate={editingCandidate} onClose={() => setEditingCandidate(null)} onSaved={fetchCandidates} />}

      {archiveTarget && <ArchiveCandidateModal candidate={archiveTarget.candidate} mode={archiveTarget.mode} onConfirm={handleConfirmArchive} onCancel={() => setArchiveTarget(null)} />}
