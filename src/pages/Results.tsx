...
import {
  BarChart3Icon, TrendingUpIcon, AlertCircleIcon, CheckCircle2Icon,
  FilterIcon, SearchIcon, XCircleIcon, ShieldCheckIcon } from
'lucide-react';
...
export function Results({ candidateId, permission = 'full' }: ResultsProps) {
  ...
  useEffect(() => {loadResults();}, [candidateId]);
  useEffect(() => {setCurrentPage(1);}, [filterStatus, searchTerm]);

  const loadResults = useCallback(async () => {
    try {
      setLoading(true);
      let query = supabase.from('results').select('*').order('id', { ascending: false });
  ...
  const processed = results.map((r) => ({ ...r, compatibility: Math.min(100, Math.round(r.score || 0)) }));
  ...
  filter((r) => filterStatus === 'all' || getCompatibilityLevel(r.compatibility) === filterStatus).
  filter((r) => {
    if (!searchTerm.trim()) return true;
    const term = searchTerm.toLowerCase();
    return (
      String(r.candidate?.name || '').toLowerCase().includes(term) ||
      String(r.candidate?.email || '').toLowerCase().includes(term) ||
      String(r.test?.name || '').toLowerCase().includes(term));
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center h-64">
      <BarChart3Icon className="w-10 h-10 animate-pulse mb-3" style={{ color: TR.blue }} />
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
        { label: 'Evaluaciones', value: processed.length, icon: BarChart3Icon, color: TR.blue },
        { label: 'Promedio', value: `${avg}%`, icon: TrendingUpIcon, color: TR.navy },
        { label: 'Muy Recomendados', value: highlyRecommended, icon: CheckCircle2Icon, color: TR.green },
        { label: 'Alertas', value: withAlerts, icon: AlertCircleIcon, color: '#ef4444' }].
        map((s) =>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-3 flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Buscar por candidato, email o prueba..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full pl-9 pr-10 py-2 border border-gray-200 rounded-lg text-sm outline-none bg-gray-50/50 focus:bg-white transition-colors" />
        ...
        <div className="flex items-center gap-2">
          ...
          className="py-2 px-3 border border-gray-200 rounded-lg text-sm outline-none bg-gray-50 focus:bg-white">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
        ...
        {/* Tabla header */}
        <div className={`grid px-6 py-3 text-[11px] font-bold uppercase tracking-widest text-gray-400 border-b border-gray-100 ${isCandidateView ? 'grid-cols-10' : 'grid-cols-12'}`}
        ...
        <div className="divide-y divide-gray-50">
          ...
          paginated.map((r) => {
            ...
            return (
              <div key={r.id}
              className={`grid px-6 py-4 items-center hover:bg-blue-50/20 transition-colors ${isCandidateView ? 'grid-cols-10' : 'grid-cols-12'}`}>
                ...
                <div className="col-span-3">
                  <div className="flex items-center gap-2">
                    <div className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: `${TR.blue}10` }}>
                      <ShieldCheckIcon className="w-3.5 h-3.5" style={{ color: TR.blue }} />
                    </div>
                ...
                <div className="col-span-1">
                  <span className="text-sm font-black" style={{ color: TR.navy }}>{Number(r.score) || 0}</span>
                </div>
                ...
                <div className="col-span-3 pr-4">
                  <div className="flex items-center gap-2">
                    <div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
                      ...
                      style={{ width: `${r.compatibility}%`, background: color }} />
                    </div>
                    <span className="text-xs font-bold w-9 text-right" style={{ color }}>{r.compatibility}%</span>
                  </div>
                </div>

                {/* Resultado */}
                <div className="col-span-3">
                  <span className="px-3 py-1.5 rounded-xl text-xs font-bold" style={{ background: bg, color }}>
