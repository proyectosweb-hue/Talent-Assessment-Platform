...
import { ClipboardListIcon, PlayIcon, FileTextIcon, EditIcon, CopyIcon, ArchiveIcon, RotateCcwIcon, LockIcon, SearchIcon, XCircleIcon, ClockIcon, CheckCircle2Icon, LayersIcon, PlusIcon, XIcon, SaveIcon, Loader2Icon, ChevronDownIcon, ChevronUpIcon, Trash2Icon } from 'lucide-react';
...
function GroupFormModal({
  ...
  useEffect(() => {
    ...
    Promise.all([supabase.from('test_group_tests').select('test_id').eq('group_id', group.id), supabase.from('test_group_candidates').select('candidate_id').eq('group_id', group.id)]).then(([tRes, cRes]) => {
  ...
  const toggleTest = (id: any) => {
    const s = toStrId(id);
    if (!isValidId(s)) return;
    setSelectedTests((p) => p.includes(s) ? p.filter((x) => x !== s) : [...p, s]);
  };
  const toggleCandidate = (id: any) => {
    const s = toStrId(id);
    setSelectedCandidates((p) => p.includes(s) ? p.filter((x) => x !== s) : [...p, s]);
  };
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (validTestIds.length === 0) {
      showToast('Selecciona al menos una prueba', 'warning');
      return;
    }
    setSaving(true);
    try {
      ...
      if (group) {
        const {
          error
        } = await supabase.from('test_groups').update({
          name: name.trim(),
          description: description.trim()
        }).eq('id', group.id);
        if (error) throw error;
        groupId = toStrId(group.id);
        await Promise.all([supabase.from('test_group_tests').delete().eq('group_id', groupId), supabase.from('test_group_candidates').delete().eq('group_id', groupId)]);
      } else {
        const {
          data,
          error
        } = await supabase.from('test_groups').insert({
          name: name.trim(),
          description: description.trim()
        }).select('id').single();
        if (error) throw error;
      ...
      const {
        error: testsError
      } = await supabase.from('test_group_tests').insert(validTestIds.map((id) => ({
        group_id: groupId,
        test_id: id
      })));
      ...
      if (selectedCandidates.length > 0) {
        ...
        } = await supabase.from('test_group_candidates').insert(selectedCandidates.filter(isValidId).map((candidate_id) => ({
          group_id: groupId,
          candidate_id
        })));
      ...
      await logAudit({
        ...
        module: 'tests',
        description: `Grupo "${name}" ${group ? 'actualizado' : 'creado'} con ${validTestIds.length} pruebas y ${selectedCandidates.length} candidatos`,
        entity_name: name
      });
  ...
  const filteredCandidates = allCandidates.filter((c) => !candidateSearch.trim() || (c.name || '').toLowerCase().includes(candidateSearch.toLowerCase()) || (c.email || '').toLowerCase().includes(candidateSearch.toLowerCase()));
  return <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50" onClick={onClose}>
      <div className="bg-white rounded-3xl shadow-2xl max-w-lg w-full overflow-hidden max-h-[92vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="relative px-7 py-6 flex items-center justify-between flex-shrink-0" style={{
        ...
        <form onSubmit={handleSave} className="flex flex-col flex-1 overflow-hidden">
          <div className="px-7 py-5 space-y-5 overflow-y-auto flex-1">
            ...
            <div>
              ...
              <input type="text" required value={name} onChange={(e) => setName(e.target.value)} className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm outline-none bg-gray-50 focus:bg-white focus:border-blue-400 focus:ring-2 focus:ring-blue-100 text-gray-900 transition-all" placeholder="Ej: Contabilidad, Ventas..." />
            </div>

            {/* Descripción */}
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-gray-400 mb-1.5">Descripción</label>
              <input type="text" value={description} onChange={(e) => setDesc(e.target.value)} className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm outline-none bg-gray-50 focus:bg-white focus:border-blue-400 focus:ring-2 focus:ring-blue-100 text-gray-900 transition-all" placeholder="Descripción opcional..." />
            </div>
            ...
            <div>
              ...
              {loadingRelations ? <div className="flex items-center gap-2 text-gray-400 text-sm py-3"><Loader2Icon className="w-4 h-4 animate-spin" /> Cargando...</div> : allTests.length === 0 ? <p className="text-sm text-gray-400 py-2">No hay pruebas disponibles</p> : <div className="space-y-2 max-h-36 overflow-y-auto pr-1">
                  {allTests.map((t) => {
                const selected = selectedTests.includes(toStrId(t.id));
                return <div key={t.id} onClick={() => toggleTest(t.id)} className="flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all select-none" style={{
                  borderColor: selected ? TR.blue : '#e5e7eb',
                  background: selected ? `${TR.blue}08` : '#fafafa'
                }}>
                        {/* Checkbox visual */}
                        <div className="w-5 h-5 rounded-md border-2 flex items-center justify-center flex-shrink-0 transition-all" style={{
                    borderColor: selected ? TR.blue : '#d1d5db',
                    background: selected ? TR.blue : 'white'
                  }}>
            ...
            <div>
              ...
              <div className="relative mb-2">
                ...
                <input type="text" placeholder="Buscar candidato..." value={candidateSearch} onChange={(e) => setCandidateSearch(e.target.value)} className="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-lg text-sm outline-none bg-gray-50 focus:bg-white" />
              </div>
              {loadingRelations ? <div className="flex items-center gap-2 text-gray-400 text-sm py-3"><Loader2Icon className="w-4 h-4 animate-spin" /> Cargando...</div> : filteredCandidates.length === 0 ? <p className="text-sm text-gray-400 py-2">{candidateSearch ? `Sin resultados para "${candidateSearch}"` : 'No hay candidatos'}</p> : <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                  {filteredCandidates.map((c) => {
                const selected = selectedCandidates.includes(toStrId(c.id));
                return <div key={c.id} onClick={() => toggleCandidate(c.id)} className="flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all select-none" style={{
                  borderColor: selected ? TR.green : '#e5e7eb',
                  background: selected ? `${TR.green}08` : '#fafafa'
                }}>
                        {/* Avatar */}
                        <div className="w-8 h-8 rounded-lg flex items-center justify-center text-white text-xs font-black flex-shrink-0" style={{
                    background: selected ? `linear-gradient(135deg, ${TR.green}, ${TR.greenDark})` : `linear-gradient(135deg, ${TR.blue}, ${TR.navy})`
                  }}>
                        ...
                        {/* Checkbox visual */}
                        <div className="w-5 h-5 rounded-md border-2 flex items-center justify-center flex-shrink-0 transition-all" style={{
                    borderColor: selected ? TR.green : '#d1d5db',
                    background: selected ? TR.green : 'white'
                  }}>
                          {selected && <svg className="w-3 h-3 text-white" fill="none" stroke="currentColor" strokeWidth="3" viewBox="0 0 24 24"><path d="M5 13l4 4L19 7" strokeLinecap="round" strokeLinejoin="round" /></svg>}
                        </div>
                      </div>;
          ...
          <div className="px-7 py-4 border-t border-gray-100 flex gap-3 bg-gray-50/50 flex-shrink-0">
            <button type="button" onClick={onClose} className="flex-1 py-2.5 border border-gray-200 rounded-xl font-semibold text-sm text-gray-600 hover:bg-gray-100 transition-colors">
              Cancelar
            </button>
            <button type="submit" disabled={saving || loadingRelations} className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl font-bold text-sm text-white shadow-md disabled:opacity-60" style={{
...
export function Tests({
  ...
  const [groupDetails, setGroupDetails] = useState<Record<string, {
    tests: string[];
    candidates: string[];
  }>>({});
  ...
  const loadAll = useCallback(async () => {
    setLoading(true);
    try {
      const [testsRes, archivedRes, groupsRes, candidatesRes] = await Promise.all([supabase.from('tests').select('*').or('archived.eq.false,archived.is.null'), supabase.from('tests').select('*').eq('archived', true), supabase.from('test_groups').select('*').order('name'), supabase.from('candidates').select('id, name, email, position').or('archived.eq.false,archived.is.null').order('name')]);
      ...
      if (gids.length > 0) {
        const [gtRes, gcRes] = await Promise.all([supabase.from('test_group_tests').select('group_id, test_id').in('group_id', gids), supabase.from('test_group_candidates').select('group_id, candidate_id').in('group_id', gids)]);
  ...
  useRealtime({
    table: 'test_group_tests',
    onChange: loadAll
  });
  useRealtime({
    table: 'test_group_candidates',
    onChange: loadAll
  });
  const loadCandidates = async () => {
    const {
      data
    } = await supabase.from('candidates').select('*').or('archived.eq.false,archived.is.null');
    setCandidates(data || []);
  };
  ...
  const handleSaveEdit = async (e: React.FormEvent) => {
    ...
    await logAudit({
      action: 'UPDATE',
      module: 'tests',
      description: `Prueba "${selectedTestForAction.name}" editada`,
      entity_name: selectedTestForAction.name
    });
  ...
  const confirmArchive = async () => {
    ...
    await logAudit({
      action: 'ARCHIVE',
      module: 'tests',
      description: `Prueba "${selectedTestForAction.name}" archivada`,
      entity_name: selectedTestForAction.name
    });
    showToast('Prueba archivada', 'success');
  ...
  const handleRestoreClick = async (test: any) => {
    ...
    await logAudit({
      action: 'UNARCHIVE',
      module: 'tests',
      description: `Prueba "${test.name}" restaurada`,
      entity_name: test.name
    });
    showToast('Prueba restaurada', 'success');
    loadAll();
  };
  const handleDuplicateTest = async (test: any) => {
    ...
    await logAudit({
      action: 'CREATE',
      module: 'tests',
      description: `Prueba "${test.name + ' (Copia)'}" duplicada`,
      entity_name: test.name + ' (Copia)'
    });
  ...
  const startTest = (candidateId: string) => {
    if (onApplyTest && selectedTestForAction) onApplyTest(selectedTestForAction.id, candidateId);
  };
  const handleDeleteGroup = async (group: any) => {
    if (!window.confirm(`¿Eliminar el grupo "${group.name}"?`)) return;
    const {
      error
    } = await supabase.from('test_groups').delete().eq('id', group.id);
    if (error) {
      showToast('Error: ' + error.message, 'error');
      return;
    }
    await logAudit({
      action: 'DELETE',
      module: 'tests',
      description: `Grupo "${group.name}" eliminado`,
  ...
  if (loading) return <div className="flex flex-col items-center justify-center h-64">
      <ClipboardListIcon className="w-10 h-10 animate-pulse mb-3" style={{
      color: TR.blue
    }} />
  ...
  return <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        {!isReadonly && activeTab === 'groups' && <button onClick={() => {
      ...
      <div className="flex gap-1 bg-gray-100 p-1 rounded-xl w-fit">
        ...
        id: 'groups',
        label: 'Grupos por Candidato',
        icon: LayersIcon
      }] as const).map((tab) => <button key={tab.id} onClick={() => setActiveTab(tab.id)} className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all" style={{
      ...
      {activeTab === 'tests' && <>
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-3">
            <div className="relative">
              ...
              <input type="text" placeholder="Buscar prueba por nombre o descripción..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-9 pr-10 py-2 border border-gray-200 rounded-lg outline-none text-sm bg-gray-50/50 focus:bg-white" />
          ...
          <div>
            ...
            {filteredTests.length === 0 ? <div className="bg-white rounded-2xl border border-gray-100 p-16 text-center">
                ...
                {filteredTests.map((test) => <div key={test.id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-md transition-all group">
                    <div className="p-5" style={{
              borderBottom: `3px solid ${TR.blue}15`
            }}>
                  ...
                  background: `${TR.blue}12`
                    ...
                    <div className="px-5 pb-5 pt-3">
                      <button onClick={() => handleApply(test)} className="w-full py-2.5 rounded-xl text-white text-sm font-bold hover:opacity-90 shadow-md flex items-center justify-center gap-2" style={{
                background: `linear-gradient(135deg, ${TR.green}, ${TR.greenDark})`,
          ...
          <div className="border-t border-gray-100 pt-4">
            <button onClick={() => setShowArchivedSection(!showArchivedSection)} className="flex items-center gap-2 text-sm font-bold text-gray-500 hover:text-gray-800">
            ...
            {showArchivedSection && archivedTests.length > 0 && <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4 mt-4">
                {archivedTests.map((test) => <div key={test.id} className="bg-gray-50 rounded-2xl border border-gray-200 overflow-hidden opacity-70 hover:opacity-100 transition-all">
                    <div className="p-5">
                      <div className="flex items-start justify-between mb-3">
      ...
      {/* ═══ TAB: GRUPOS ═══ */}
      {activeTab === 'groups' && <div className="space-y-4">
          {groups.length === 0 ? <div className="bg-white rounded-2xl border border-gray-100 p-16 text-center">
          ...
          setEditingGroup(null);
          setShowGroupModal(true);
        }} className="mt-4 flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold text-white shadow-md mx-auto" style={{
          background: `linear-gradient(135deg, ${TR.blue}, ${TR.navy})`
        }}>
                  <PlusIcon className="w-4 h-4" /> Crear primer grupo
                </button>}
            </div> : groups.map((group) => {
        const detail = groupDetails[toStrId(group.id)] || {
          tests: [],
          candidates: []
        };
        ...
        return <div key={group.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div className="px-6 py-4 flex items-center justify-between border-b border-gray-100" style={{
            background: `linear-gradient(135deg, ${TR.navy}06, ${TR.blue}06)`
                ...
                background: `${TR.blue}12`
                      ...
                      <LayersIcon className="w-5 h-5" style={{
                  color: TR.blue
                }} />
                    </div>
                    <div>
                ...
                color: TR.blue
                      ...
                      <button onClick={() => handleDeleteGroup(group)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold text-red-600 bg-red-50 hover:bg-red-100">
                ...
                <div className="px-6 py-4 grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div>
                    ...
                    {groupTests.length === 0 ? <p className="text-xs text-gray-300 italic">Sin pruebas asignadas</p> : <div className="space-y-1.5">
                        {groupTests.map((t) => <div key={t.id} className="flex items-center gap-2 px-3 py-2 rounded-lg" style={{
                  background: `${TR.blue}06`
                }}>
                            <ClipboardListIcon className="w-3.5 h-3.5 flex-shrink-0" style={{
                    color: TR.blue
                  }} />
                  ...
                  <div>
                    ...
                    {groupCands.length === 0 ? <p className="text-xs text-gray-300 italic">Sin candidatos asignados</p> : <div className="space-y-1.5 max-h-40 overflow-y-auto">
                        {groupCands.map((c) => <div key={c.id} className="flex items-center gap-2 px-3 py-2 rounded-lg" style={{
                  background: `${TR.green}08`
                }}>
                            <div className="w-6 h-6 rounded-md flex items-center justify-center text-white text-[10px] font-black flex-shrink-0" style={{
                    background: `linear-gradient(135deg, ${TR.green}, ${TR.greenDark})`
                  }}>
                              {String(c.name || '').substring(0, 2).toUpperCase()}
                            </div>
      ...
      {/* Modal aplicar */}
      {showApplyModal && selectedTestForAction && <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
            <div className="px-6 py-5 border-b border-gray-100" style={{
          background: `linear-gradient(135deg, ${TR.blue}08, ${TR.green}08)`
        }}>
            ...
            <div className="p-5">
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {filteredCandidates.length === 0 ? <p className="text-center text-gray-400 py-6 text-sm">No se encontraron candidatos</p> : filteredCandidates.map((c) => <button key={c.id} onClick={() => startTest(c.id)} className="w-full flex items-center gap-3 p-3 border border-gray-100 rounded-xl hover:border-blue-200 hover:bg-blue-50/50 transition-all text-left group">
                      <div className="w-9 h-9 rounded-lg flex items-center justify-center text-white text-xs font-bold flex-shrink-0" style={{
                background: `linear-gradient(135deg, ${TR.blue}, ${TR.navy})`
              }}>
                        {String(c.name || '').substring(0, 2).toUpperCase()}
                      </div>
      ...
      {/* Modal editar prueba */}
      {showEditModal && selectedTestForAction && <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
            ...
            <form onSubmit={handleSaveEdit} className="p-5 space-y-4">
              <div>
                ...
                <input className="w-full border border-gray-200 rounded-xl p-3 text-sm outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100" value={selectedTestForAction.name} onChange={(e) => setSelectedTestForAction({
              ...
              <div className="flex gap-3">
                ...
                <button type="submit" className="flex-1 py-2.5 rounded-xl text-white font-semibold text-sm" style={{
              background: TR.blue
            }}>Guardar</button>
              </div>
            </form>
      ...
      {showArchiveModal && selectedTestForAction && <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
      ...
      {/* Modal grupo */}
      {showGroupModal && <GroupFormModal group={editingGroup} allTests={tests} allCandidates={allCandidates} onClose={() => setShowGroupModal(false)} onSaved={loadAll} />}
    </div>;
}