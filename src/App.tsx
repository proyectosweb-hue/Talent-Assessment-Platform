...
import {
  ClipboardListIcon, PlayIcon, BarChart3Icon,
  AlertCircleIcon, CheckCircle2Icon } from
'lucide-react';
...
export interface ModulePermission {module: string;label: string;level: PermissionLevel;}
export interface PermissionsMap {[module: string]: PermissionLevel;}
...
function useAppSettings() {
  ...
  useEffect(() => {
    supabase.from('app_settings').select('logo_url, company_name').
...
function CandidateTestsView({ candidateRecordId, onStartTest
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        from('test_group_candidates').
        select('group_id').
        eq('candidate_id', String(candidateRecordId));
        ...
        if (groupCandidates && groupCandidates.length > 0) {
          ...
          const { data: groupTests } = await supabase.
          from('test_group_tests').select('test_id').in('group_id', groupIds);
          ...
          if (groupTests && groupTests.length > 0) {
            ...
            from('tests').select('*').or('archived.eq.false,archived.is.null');
            ...
            allowedTestIds.includes(String(t.id))
            );
        ...
        from('results').select('test_id').eq('user_name', candidateRecordId);
  ...
  if (loading || candidateRecordId === null) return (
  ...
  return (
    <div className="space-y-8">
      ...
      style={{ background: 'linear-gradient(135deg, #1a2d6b 0%, #2D4494 60%, #3a55b5 100%)' }}>
        <div className="px-8 py-8 relative">
          ...
          <div className="relative z-10">
            ...
            <p className="text-white/50 text-sm">
              {pending > 0 ?
              `Tienes ${pending} prueba${pending > 1 ? 's' : ''} pendiente${pending > 1 ? 's' : ''}` :
            ...
            <div className="mt-5 flex items-center gap-4">
                ...
                style={{ width: `${progress}%`, background: 'linear-gradient(90deg, #7DB928, #5e8c1e)' }} />
                </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-16 text-center shadow-sm">
          <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4" style={{ background: '#2D449412' }}>
            <ClipboardListIcon className="w-8 h-8" style={{ color: '#2D4494' }} />
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          return (
            ...
            style={{ borderColor: done ? '#7DB92830' : '#e5e7eb' }}>
                <div className="flex">
                  {/* Franja lateral */}
                  <div className="w-1.5 flex-shrink-0"
                style={{ background: done ? 'linear-gradient(180deg, #7DB928, #5e8c1e)' : 'linear-gradient(180deg, #2D4494, #1a2d6b)' }} />
                  <div className="flex-1 p-6">
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1 min-w-0 pr-3">
                        {done &&
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider mb-1"
                      style={{ background: '#7DB92815', color: '#5e8c1e' }}>
                            <CheckCircle2Icon className="w-3 h-3" /> Completada
                          </span>
                      }
                        <h3 className="font-black text-gray-900 leading-tight">{String(test.name || '')}</h3>
                        {test.description &&
                      <p className="text-xs text-gray-400 mt-1 line-clamp-2">{String(test.description)}</p>
                      }
                      </div>
                      <div className="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0"
                    style={{ background: done ? '#7DB92812' : '#2D449412' }}>
                        {done ?
                      <CheckCircle2Icon className="w-6 h-6" style={{ color: '#7DB928' }} /> :
                      <ClipboardListIcon className="w-6 h-6" style={{ color: '#2D4494' }} />}
                      </div>
                    </div>
                    <div className="flex items-center gap-3 mb-4 pt-3 border-t border-gray-50">
                      ...
                      {test.duration &&
                    <span className="text-xs text-gray-400 font-medium">· ⏱ {test.duration} min</span>
                    }
                    </div>
                    {done ?
                  <div className="w-full py-3 rounded-xl font-bold text-sm text-center flex items-center justify-center gap-2"
                  style={{ background: '#7DB92810', color: '#5e8c1e', border: '1px solid #7DB92830' }}>
                        <CheckCircle2Icon className="w-4 h-4" /> Prueba Completada
                      </div> :

                  <button onClick={() => {if (!candidateRecordId) return;onStartTest(test.id, candidateRecordId);}}
                  className="w-full py-3 rounded-xl font-black text-sm text-white flex items-center justify-center gap-2 hover:opacity-90 active:scale-95"
                  style={{ background: 'linear-gradient(135deg, #2D4494, #1a2d6b)', boxShadow: '0 4px 14px rgba(45,68,148,0.3)' }}>
...
// ── Portal candidato ──────────────────────────────────────────────
function CandidatePortal({ onLogout, candidateRecordId, permissions
  ...
  const handleStartTest = (testId: string, candidateId: string) => {
    setTestId(testId);setCandidateId(candidateId);setCurrentPage('test-application');
  };

  if (currentPage === 'test-application' && testId && candidateId)
  return <TestApplication testId={testId} candidateId={candidateId} />;

  const moduleLabels: Record<string, string> = {
    tests: 'Mis Pruebas', results: 'Mis Resultados'
  };
      ...
      <header className="bg-white border-b border-gray-100 shadow-sm sticky top-0 z-10">
        <div className="max-w-5xl mx-auto flex items-center justify-between px-6 h-16">
          ...
          <div className="flex items-center gap-3">
            ...
            style={{ background: 'linear-gradient(135deg, #1a2d6b, #2D4494)' }}>
          ...
          <nav className="hidden md:flex items-center gap-1 bg-gray-100 p-1 rounded-xl">
            ...
            <button key={mod.module} onClick={() => setCurrentPage(mod.module)}
          ...
          <div className="flex items-center gap-3">
            ...
            style={{ background: 'linear-gradient(135deg, #2D4494, #7DB928)' }}>
              {initials}
            </div>
            <button onClick={onLogout}
            className="px-3 py-1.5 text-sm font-bold text-red-600 border border-red-200 rounded-xl hover:bg-red-50 transition-colors">
              Salir
            </button>
          </div>
        </div>
...
function AppContent() {
  ...
  const resolveCandidateRecord = async (email: string) => {
    try {
      const { data } = await supabase.from('candidates').select('id').eq('email', email).single();
      setCandidateRecordId(data?.id ? String(data.id) : null);
    } catch {setCandidateRecordId(null);}
  };
  ...
  const loadRolePermissions = async (roleName: string) => {
    try {
      ...
      if (data?.permissions) {
        ...
        perms.forEach((p) => {map[p.module] = p.level;});
        setPermissionsMap(map);
      }
    } catch {setRolePermissions([]);setPermissionsMap({});}
  };
    ...
    if (stored) {
      try {
        ...
        if (Date.now() - (parsed.loginAt || 0) > SESSION_DURATION) {
          localStorage.removeItem('user');setLoading(false);return;
        }
  ...
  const handleCandidateAdded = useCallback(() => {
    setShowCandidateModal(false);setRefreshKey((p) => p + 1);
  }, []);
  ...
  const handleLogout = async () => {
    await logAudit({ action: 'LOGOUT', module: 'session', description: 'Cierre de sesión' });
    localStorage.removeItem('user');
    setIsAuthenticated(false);setUserRole('');setCandidateRecordId(null);
    setRolePermissions([]);setPermissionsMap({});setCurrentPage('dashboard');
  };
  ...
  if (userRole === 'candidato') return (
    <CandidatePortal onLogout={handleLogout} candidateRecordId={candidateRecordId} permissions={rolePermissions} />);


  const handleViewCandidate = (candidate: Candidate) => {
    setSelectedCandidate(candidate);setCurrentPage('candidate-detail');
  };
  ...
  const renderPage = () => {
    ...
    return <CandidateDetail candidate={selectedCandidate}
    onBack={() => {setSelectedCandidate(null);setCurrentPage('candidates');}}
    onAddNew={() => setShowCandidateModal(true)} />;
    return (
      <Layout currentPage={currentPage} onNavigate={setCurrentPage} onLogout={handleLogout} permissionsMap={permissionsMap}>
