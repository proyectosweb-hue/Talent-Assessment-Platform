...
export function Pagination({
  ...
  return <div className="flex items-center justify-between px-2 py-3">
      ...
      <div className="flex items-center gap-1">
        ...
        {getPages().map((page, i) => page === '...' ? <span key={`dots-${i}`} className="px-2 text-gray-400 text-sm">
              …
            </span> : <button key={page} onClick={() => onPageChange(page as number)} className={`w-9 h-9 rounded-lg text-sm font-medium transition-colors ${currentPage === page ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-100'}`}>
              {page}
            </button>)}
