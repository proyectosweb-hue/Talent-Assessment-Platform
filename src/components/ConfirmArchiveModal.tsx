...
export function ConfirmArchiveModal({
  ...
  return <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50" onClick={onCancel}>
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        background: isArchive ? 'linear-gradient(135deg, #92400e, #b45309)' : `linear-gradient(135deg, ${TR.greenDark}, ${TR.green})`
      }}>
          ...
          <div className="flex items-center gap-3 relative z-10">
            <div className="w-11 h-11 rounded-xl flex items-center justify-center bg-white/20">
              {isArchive ? <ArchiveIcon className="w-5 h-5 text-white" /> : <ArchiveRestoreIcon className="w-5 h-5 text-white" />}
            </div>
            <div>
              <h2 className="text-lg font-black text-white">{isArchive ? 'Archivar Puesto' : 'Desarchivar Puesto'}</h2>
              <p className="text-xs text-white/60 mt-0.5">{isArchive ? 'El puesto dejará de aparecer activo' : 'El puesto volverá a la lista activa'}</p>
            </div>
          </div>
        ...
        <div className="p-6">
          <p className="text-gray-600 text-sm leading-relaxed">
            {isArchive ? <>¿Estás seguro de archivar el puesto <strong className="text-gray-900 font-bold">"{positionName}"</strong>? No aparecerá en la lista activa.</> : <>¿Deseas restaurar el puesto <strong className="text-gray-900 font-bold">"{positionName}"</strong> a la lista activa?</>}
          </p>
        </div>
        ...
        <div className="px-6 pb-6 flex gap-3">
          ...
          background: isArchive ? 'linear-gradient(135deg, #b45309, #92400e)' : `linear-gradient(135deg, ${TR.green}, ${TR.greenDark})`
        }}>
            {isArchive ? 'Sí, archivar' : 'Sí, restaurar'}
          </button>
        </div>
      </div>
    </div>;
